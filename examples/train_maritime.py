...
if __name__ == "__main__":
    ...
    #TODO rework so agents that are not training don't require a full sized buffer
    for aid in args.policies:
        if args.policies[aid] == "init_ppo":
            ...
            print(f"PPO Creation {aid} Obs space: {act_spaces[aid]}")
            policies[aid] = ppo.PPO(obs_spaces[aid], act_spaces[aid], config)
        else:
            policies[aid] = policies[args.policies[aid]] # Agents 3-5 use the same policy from Agents 0-2
    ...
    for iteration in range(1, args.num_iterations+1):
        ...
        #Bootstrap values in all buffers via GAE
        for aid in buffers:
        ...
        #Log information about training iteration
